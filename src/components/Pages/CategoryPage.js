...
export const CategoryPage = () => {
  ...
  const IconComponent = category?.icon || Circuitry;
    ...
    if (items.length === 0) {
      return (
        <div className="no-search-results">
          ...
          <p>No encontramos productos {subcategory ? `en "${subcategory}"` : `en "${category?.name}"`}.</p>
        </div>
      );
    }
  ...
  const getFilters = () => {
    ...
    // Add category filter
    if (subcategory) {
      filters += ` AND categorias_flat:"${subcategory}"`;
    } else if (category) {
      filters += ` AND categorias_flat:"${category.name}"`;
    }
    
    // Add store filter if enabled
    if (storeFilter) {
              ...
              <div>
                ...
                {subcategory && (
                  <p className="category-breadcrumb">
                    <Link to={`/categoria/${categoryId}`}>{category.name}</Link> / {subcategory}
                  </p>
                )}
              </div>
            </div>
